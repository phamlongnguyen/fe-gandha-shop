...
// Reusable Tweaks shell + form-control helpers. Owns the host protocol
// (__activate_edit_mode / __deactivate_edit_mode) so prototypes don't re-roll it.
...
export function useTweaks<T extends TweakValues>(defaults: T): [T, (key: keyof T | Partial<T>, val?: unknown) => void] {
  const [values, setValues] = useState<T>(defaults);

  const setTweak = useCallback((keyOrEdits: keyof T | Partial<T>, val?: unknown) => {
    const edits =
      typeof keyOrEdits === 'object' && keyOrEdits !== null ? (keyOrEdits as Partial<T>) : ({ [keyOrEdits]: val } as Partial<T>);
